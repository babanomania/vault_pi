...
try:
    ...
    fontSmall = ImageFont.truetype(os.path.join(picdir, 'Font.ttc'), 14)
    ...
    uptime = subprocess.check_output('uptime | awk -F"," \'{ print $1 }\' | awk -F" up " \'{ print $2 }\'', shell=True, text=True)
    x_start = epd.height - ( 7 * len(uptime) )
    drawry.text((x_start, 84), uptime, font = fontSmall, fill = 0)
